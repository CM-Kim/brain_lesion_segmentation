...
i=1;total=5
temp = np.ones_like( masks[0, :, :, 24] )
for idx in np.random.randint(0,high=images.shape[0],size=total):
    plt.subplot(total,3,i);i+=1
    plt.imshow( np.squeeze(images[idx, :, :, 24],axis=-1), cmap='gray' )
    plt.title("MRI Image");plt.axis('off')
    
    plt.subplot(total,3,i);i+=1
    plt.imshow( np.squeeze(images[idx, :, :, 24],axis=-1), cmap='gray' )
    plt.imshow( np.squeeze(masks[idx, :, :, 24],axis=-1), alpha=0.8, cmap='Reds' )
    plt.title("Original Mask");plt.axis('off')
    
    plt.subplot(total,3,i);i+=1
    plt.imshow( np.squeeze(images[idx, :, :, 24],axis=-1), cmap='gray' )
    plt.imshow( np.squeeze(predict[idx, :, :, 24],axis=-1),  alpha=0.8, cmap='Reds' )
